...
describe('Yoshi Utils', () => {
  ...
  describe('.usingYarn()', () => {
    it('should return true if it was executed with yarn', () => {
      process.env['npm_config_user_agent'] = 'yarn/0.27.5 npm/? node/v6.11.1 darwin x64';
      expect(usingYarn()).to.equal(true);
    });

    it('should return false if it was executed with npm', () => {
      process.env['npm_config_user_agent'] = 'npm/3.10.10 node/v6.11.1 darwin x64';
  ...
  describe('.pkgManager()', () => {
    ...
    it('should return current package manager', () => {
      const pm = process.env.npm_execpath = '/Users/tomas/.nvm/versions/node/v6.11.1/lib/node_modules/yarn/bin/yarn.js';
